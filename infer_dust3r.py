...
# Create image pairs
def make_sparse_pairs(images, step):
    return [(images[i], images[i + step]) for i in range(0, len(images) - step, step)]

def main():
    # === Parameters ===
    parser = argparse.ArgumentParser(description="Dust3r 3D Reconstruction Inference")
    parser.add_argument("--scenes_root", type=str, default="7SCENES",
                        help="Directory containing scene data")
    parser.add_argument("--output_dir", type=str, default="PLY/seq",
                        help="Output directory for PLY files")
    parser.add_argument("--model_ckpt", type=str, 
                        default="checkpoints/dust3r_7scenes/train_3000/checkpoint-best.pth",
                        help="Path to model checkpoint")
    parser.add_argument("--step", type=int, default=20,
                        help="Step size for creating image pairs")
    parser.add_argument("--voxel_size", type=float, default=7.5e-3,
                        help="Voxel size for point cloud downsampling")
    parser.add_argument("--seq_pattern", type=str, default="seq-*",
                        help="Pattern for sequence directories (e.g., 'seq-*', 'sparse-seq-*')")
    
    args = parser.parse_args()

    # === Initialize model ===
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = AsymmetricCroCo3DStereo.from_pretrained(args.model_ckpt).to(device).eval()

    # Output directory
    os.makedirs(args.output_dir, exist_ok=True)

    # Get each sequence directory using the specified pattern
    scene_dirs = sorted(glob(os.path.join(args.scenes_root, "*", "test", args.seq_pattern)))
    ...
    for seq_dir in tqdm(scene_dirs, desc="Processing Sequences"):
        ...
        pairs = make_sparse_pairs(images, step=args.step)
        ...
        # Save as binary .ply
        ply_path = os.path.join(args.output_dir, f"{scene}-{seq}.ply")
